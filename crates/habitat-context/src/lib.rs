...
pub struct ContextItem {
    ...
    pub original_evidence_ref: Option<String>,
}
impl ContextItem {
    pub fn new(
        id: &str,
    ...
    pub fn original(mut self, evidence_ref: &str) -> Self {
        self.original_evidence_ref = Some(evidence_ref.into());
        self
    }
    pub fn source_access(&self, max_bytes: usize, expires_at: u64) -> Option<SourceAccess> {
        self.original_evidence_ref
            .as_ref()
            .filter(|_| self.consequential)
            .map(|reference| SourceAccess {
    ...
    pub fn contradiction(id: &str, claims: &[&str], provenance: Provenance) -> Self {
        Self::new(
            id,
            TruthClass::UnresolvedUncertainty,
            &claims.join(" | "),
            provenance,
            true,
        )
...
pub struct Omission {
    pub item_id: String,
    pub reason: String,
    pub resulting_uncertainty: String,
}
...
impl Compiler {
    pub fn new(budget: usize, compiled_at: u64) -> Self {
        Self {
            budget,
            compiled_at,
        }
    ...
    pub fn compile(
        ...
        for item in &mut items {
            item.provenance.compiled_at = self.compiled_at;
        }
            ...
            if used + size <= self.budget {
                ...
                omissions.push(Omission {
                    ...
                    resulting_uncertainty:
                        "required detail omitted; action may require a context request".into(),
                })
            }
        }
        let mut bundle = ContextBundle {
            ...
            predecessor_id: predecessor.map(Into::into),
        ...
        bundle.id = format!(
            "context:sha256:{:x}",
            Sha256::digest(serde_json::to_vec(&bundle).unwrap())
        );
        Ok(bundle)
    }
}
...
impl ContextRequest {
    pub fn validate(&self, max_depth: u8) -> Result<(), ContextError> {
        ...
        if self.materiality.trim().is_empty() {
            return Err(ContextError::NonMaterial);
        }
        if self.depth > max_depth {
            return Err(ContextError::RecursionBound);
        }
        Ok(())
    }
}
...
impl SkillDescriptor {
    pub fn applicable(&self, condition: &str) -> bool {
        !self.use_when.is_empty()
            && !self.do_not_use_when.is_empty()
            && !self.inputs.is_empty()
            && !self.outputs.is_empty()
            && !self.termination_conditions.is_empty()
            && condition.contains(&self.use_when)
            && !condition.contains(&self.do_not_use_when)
    }
}
...
impl Broker {
    pub fn resolve(
        &self,
        compiler: &Compiler,
        predecessor: &ContextBundle,
        request: &ContextRequest,
        item: ContextItem,
    ) -> Result<ContextBundle, ContextError> {
        request.validate(self.max_depth)?;
        if !request.allowed_sources.is_empty()
            && !request.allowed_sources.contains(&item.provenance.source)
        {
            return Err(ContextError::SourceForbidden);
        }
        if item.provenance.fresh_until < request.freshness_deadline {
            return Err(ContextError::StaleRequired);
        }
        let mut bundle = compiler.compile(
...
pub fn isolate_external_content(
    id: &str,
    content: &str,
    source: &str,
    version: &str,
    ...
    ContextItem::new(
        id,
        TruthClass::UntrustedExternalData,
        content,
        Provenance::new(source, version, observed, observed),
        false,
    )
}

pub fn ingest_external_content(
    id: &str,
    content: &str,
    source: &str,
    version: &str,
    ...
    (
        isolate_external_content(id, content, source, version, observed),
        SecurityObservation {
            item_id: id.into(),
            attempted_directive: attempted,
            isolated: true,
        },
    )
}
