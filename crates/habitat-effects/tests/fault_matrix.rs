...
fn disconnect_after_dispatch_becomes_unknown_and_reconciles_without_retry(){
    ...
    dispatch(&mut ledger,&effect,Attempt::new("sha256:req",100,"mail","transport:7")).unwrap();
    ...
    ledger.begin_reconciliation(&effect.effect_id,
        ReconciliationAttempt::new("sha256:lookup",101,"mail","lookup:7")).unwrap();
    ...
    assert_eq!(ledger.attempts(&effect.effect_id).len(),1);
    assert_eq!(ledger.attempts(&effect.effect_id)[0].terminal_classification,Some(EffectState::OutcomeUnknown));
    assert_eq!(ledger.reconciliations(&effect.effect_id).len(),1);
    assert_eq!(ledger.reconciliations(&effect.effect_id)[0].observation_source.as_deref(),Some("provider-lookup"));
    assert_eq!(ledger.reconciliations(&effect.effect_id)[0].terminal_classification,Some(EffectState::ResolvedSucceeded));
}
...
fn acknowledgement_is_not_success_without_the_declared_observation(){
    ...
    dispatch(&mut ledger,&effect,Attempt::new("sha256:req",100,"mail","transport:8")).unwrap();
...
fn cancellation_and_compensation_preserve_truthful_distinct_histories(){
    ...
    ledger.cancel(&before.effect_id).unwrap(); assert_eq!(ledger.get(&before.effect_id).unwrap().state,EffectState::Rejected);
    let original=proposed(&mut ledger,"intent:original:0001");
    dispatch(&mut ledger,&original,Attempt::new("sha256:o",100,"mail","transport:o")).unwrap();
