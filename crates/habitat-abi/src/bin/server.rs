...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let socket = PathBuf::from(args.next().ok_or("usage: habitat-abi-server SOCKET LEDGER")?);
    let ledger = PathBuf::from(args.next().ok_or("usage: habitat-abi-server SOCKET LEDGER")?);
