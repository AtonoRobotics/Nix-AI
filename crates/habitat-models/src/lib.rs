...
pub struct ActivationEnvelope {
    ...
    pub visible_capabilities: Vec<CapabilityDescriptor>,
    pub deadline: u64,
    pub trace_id: String,
...
impl DispositionValidator {
    pub fn new(envelope: &ActivationEnvelope) -> Self {
        Self {
    ...
    pub fn validate(&self, candidate: CandidateOutput) -> Result<CandidateOutput, ModelError> {
        ...
        if disposition.command_id.trim().is_empty() {
            return Err(ModelError::CommandIdRequired);
        }
        if disposition.activation_id != self.envelope.activation_id {
            return Err(ModelError::ActivationMismatch);
        }
        if disposition.kind == DispositionKind::CapabilityInvocation {
            let capability = disposition
                ...
                .ok_or(ModelError::CapabilityInvisible)?;
            let operation = disposition
                .payload
                .get("operation")
                .and_then(Value::as_str)
                .ok_or(ModelError::CapabilityInvisible)?;
        ...
        if disposition.kind == DispositionKind::CompletionClaim {
            ...
            if !objective
                .map(|o| self.envelope.objective_ids.iter().any(|known| known == o))
                .unwrap_or(false)
                || evidence.map(|e| e.is_empty()).unwrap_or(true)
            {
                return Err(ModelError::InvalidCompletion);
            }
        }
        Ok(candidate)
    }
...
impl ModelDriver {
    ...
    pub fn accept(&self, candidate: CandidateOutput, now: u64) -> Result<DriverResult, ModelError> {
        ...
        if self.status == ActivationStatus::Cancelled {
        ...
        let status = if candidate.disposition.kind == DispositionKind::CompletionClaim {
            ActivationStatus::CompletionClaimed
        } else {
    ...
    pub fn check_deadline(&self, now: u64) -> Result<(), ModelError> {
        if now > self.envelope.deadline {
            Err(ModelError::LeaseExpired)
        } else {
            Ok(())
        }
    }
    pub fn cancel(&mut self, command: &str, reason: &str) -> Result<(), ModelError> {
...
impl OpenAiAdapter {
    pub fn translate(response: &Value) -> Result<CandidateOutput, ModelError> {
        let call = response
            .get("output")
            .and_then(Value::as_array)
            .and_then(|items| {
                items.iter().find(|item| {
                    item.get("type").and_then(Value::as_str) == Some("function_call")
                        && item.get("name").and_then(Value::as_str) == Some("submit_disposition")
                })
            })
            .ok_or(ModelError::MissingDisposition)?;
        let raw = call
            .get("arguments")
            .and_then(Value::as_str)
            .ok_or(ModelError::MalformedDisposition)?;
        let disposition =
            serde_json::from_str(raw).map_err(|_| ModelError::MalformedDisposition)?;
        candidate(response, disposition, "openai")
    }
}
...
impl AnthropicAdapter {
    pub fn translate(response: &Value) -> Result<CandidateOutput, ModelError> {
        let call = response
            ...
            .ok_or(ModelError::MissingDisposition)?;
        let disposition = serde_json::from_value(
            call.get("input")
                .cloned()
                .ok_or(ModelError::MalformedDisposition)?,
        )
...
fn candidate(
    ...
    provider: &str,
) -> Result<CandidateOutput, ModelError> {
    let usage = response.get("usage").ok_or(ModelError::InvalidEnvelope)?;
    Ok(CandidateOutput {
        disposition,
        provider_request_id: string(response, "id")?,
        provider: provider.into(),
...
fn number(value: &Value, key: &str) -> Result<u64, ModelError> {
    value
        .get(key)
