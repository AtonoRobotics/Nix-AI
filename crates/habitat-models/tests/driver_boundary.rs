...
fn envelope() -> ActivationEnvelope {
    ActivationEnvelope {
        ...
        activation_id: "activation:9".into(),
        agent_id: "agent:9".into(),
        objective_ids: vec!["objective:9".into()],
        context_bundle_id: "context:9".into(),
        ...
        correlation_id: "correlation:9".into(),
    }
}
fn candidate(kind: DispositionKind, command: &str, payload: serde_json::Value) -> CandidateOutput {
    CandidateOutput {
        disposition: Disposition {
            ...
            decision: DecisionArtifact {
                summary: "bounded decision".into(),
                evidence_refs: vec!["evidence:9".into()],
            },
        },
        provider_request_id: "provider-request:9".into(),
        provider: "test".into(),
        model: "model:9".into(),
        input_tokens: 10,
        output_tokens: 4,
...
fn validator_rejects_missing_identity_and_invisible_capability_without_heuristics() {
    ...
    assert_eq!(
        validator.validate(candidate(DispositionKind::Checkpoint, "", json!({}))),
        Err(ModelError::CommandIdRequired)
    );
    assert_eq!(
        validator.validate(candidate(
            DispositionKind::CapabilityInvocation,
            "command:1",
            json!({"capability":"admin.root","operation":"write"})
        )),
        Err(ModelError::CapabilityInvisible)
    );
    ...
    assert_eq!(
        OpenAiAdapter::translate(&prose),
        Err(ModelError::MissingDisposition)
    );
}
...
fn provider_stop_does_not_complete_but_validated_completion_claim_does() {
    ...
    let completion = driver
        .accept(
            candidate(
                DispositionKind::CompletionClaim,
                "command:2",
                json!({"objective_id":"objective:9","evidence_refs":["evidence:result"]}),
            ),
            110,
        )
        .unwrap();
...
fn cognition_evidence_is_digest_only_and_contains_no_transport_secret_or_prompt() {
    let evidence = ModelEvidence::record(
        &envelope(),
        "openai",
        "gpt-5",
        "provider-request:9",
