...
fn every_request_is_authenticated_and_mediated_against_all_grant_bounds() {
    ...
    assert_eq!(authority.evaluate(&Invocation { operation: "write".into(), ..request.clone() })
        .denial_code.as_deref(), Some("OPERATION_DENIED"));
    assert_eq!(authority.evaluate(&Invocation { generation: "generation:02".into(), ..request })
        .denial_code.as_deref(), Some("GENERATION_MISMATCH"));
}
