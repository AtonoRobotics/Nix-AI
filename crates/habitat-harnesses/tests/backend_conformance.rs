...
fn disposition() -> serde_json::Value {
    json!({"activation_id":"activation:11","command_id":"command:11",
...
fn codex_and_claude_emit_the_same_semantic_abi_and_identity() {
    let prepared = HarnessAdapter::prepare(
        &envelope(),
        "activation-set:sha256:adapter",
        &["grant:read"],
    );
    let codex =
        json!({"type":"habitat.disposition","session_id":"codex:11","payload":disposition()});
    let claude =
        json!({"type":"result","session_id":"claude:11","structured_output":disposition()});
