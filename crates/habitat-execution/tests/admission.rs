...
fn profile_must_admit_runtime_and_resources_before_execution() {
    ...
    assert_eq!(profile.admit(&IsolationRequest { cpu_cores: 3, ..native.clone() }),
               Err(AdmissionError::CapacityExceeded));
    assert_eq!(profile.admit(&IsolationRequest { runtime: Runtime::Wasi, ..native.clone() }),
...
fn native_and_oci_boundaries_are_default_deny_by_construction() {
    ...
    assert!(!spec.mounts.iter().any(|path| path.contains("provider") || path.contains("evidence")));
    assert_eq!(OciImage::parse("registry/image:latest"), Err(ImageError::DigestRequired));
    assert!(OciImage::parse("registry/image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa").is_ok());
}
