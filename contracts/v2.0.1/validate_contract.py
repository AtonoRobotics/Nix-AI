...
CONTRACT_PATH = ROOT / "nix-ai-v2.0.1.contract.json"
...
def validate_structure(contract: dict, errors: list[str]) -> None:
    ...
    require(metadata.get("version") == "2.0.1", "contract version mismatch", errors)
...
def main() -> int:
    ...
    try:
        ...
        errors.append(f"schema validation failed: {error.message}")
    ...
    for requirement in requirements:
        if requirement["source_authority"] != "V2_DERIVATION":
            continue
        source = requirement["source_reference"]
        require(
            source in requirement_set,
            f"{requirement['id']} unknown source reference {source}",
            errors,
        )
        require(source != requirement["id"], f"{requirement['id']} self derivation", errors)
        if source in requirement_set:
            derivation_edges.append((source, requirement["id"]))
    acyclic(requirement_set, derivation_edges, "requirement derivation graph", errors)
    ...
    for packet in packets:
        for relation in ("cannot_begin", "cannot_integrate", "cannot_pass"):
            combined_edges.extend(
                (dependency, packet["id"]) for dependency in packet[relation]
            )
    acyclic(packet_set, combined_edges, "combined dependency graph", errors)
