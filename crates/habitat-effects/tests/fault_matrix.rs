...
fn disconnect_after_dispatch_becomes_unknown_and_reconciles_without_retry(){
    ...
    assert_eq!(ledger.attempts(&effect.effect_id).len(),1);
    assert_eq!(ledger.attempts(&effect.effect_id)[0].observation_source.as_deref(),Some("provider-lookup"));
    assert_eq!(ledger.attempts(&effect.effect_id)[0].terminal_classification,Some(EffectState::ResolvedSucceeded));
}
