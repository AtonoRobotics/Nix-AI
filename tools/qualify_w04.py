...
def main():
 ...
 r.command(["validate-contracts"],artifacts=[a.root/"contracts/v2.0.1/nix-ai-v2.0.1.contract.json"],assertion="authority contract validates")
 ...
 for binary in binaries:r.command([binary],artifacts=[binary,a.library],assertion=f"authority adversarial binary {binary.name} passes")
 ...
 reports={"authority-report":report};write_reports(a.evidence_dir,reports);print(json.dumps(r.result(reports),indent=2,sort_keys=True))
