...
fn parent() -> Grant {
    ...
    .target_prefix("resource/group/")
    .valid_between(100, 1000)
...
fn delegation_can_only_reduce_every_parent_bound() {
    for operations in [["read"], ["write"]] {
        for expiry in [500, 999] {
            for quota in [1, 100] {
                ...
                authority
                    .issue(parent(), IndependentApproval::verified("operator:01"))
                    .unwrap();
                let child = Grant::builder(
                    "grant:child",
                    "activation:parent",
                    "activation:child",
                    "ToolCap",
                ...
                .target_prefix("resource/group/item/")
                .valid_between(100, expiry)
                .generation("generation:01")
                .delegation_depth(2)
                .quota(quota)
                .build()
    ...
    .unwrap();
    assert_eq!(
        authority.delegate("grant:parent", widened),
...
fn revocation_outage_self_authority_and_enforcement_bypass_fail_closed() {
    ...
    let mut authority =
        Authority::open(&ledger, "policy:v2", "generation:01", "state:9", 200).unwrap();
    let request = Invocation::new(
        "command:01",
        MachineId::new("machine:01").unwrap(),
        ServiceId::new("service:runtime").unwrap(),
        ActivationId::new("activation:parent").unwrap(),
        "ToolCap",
        "write",
    ...
    authority
        ...
        .unwrap();
    assert_eq!(
        authority.issue(parent(), IndependentApproval::verified("activation:parent")),
    ...
    let enforced = request
        .clone()
        .with_enforcement(EnforcementProof::verified("lsm:habitat"));
    assert!(
        authority
            .evaluate_peer(&channel, &enforced)
            .unwrap()
            .allowed
    );
    ...
    .operations(["read"])
    .target_prefix("resource/group/item/")
    .valid_between(100, 500)
    .generation("generation:01")
    .delegation_depth(2)
    .build()
    .unwrap();
    assert_eq!(
        authority.delegate("grant:parent", child),
        Err(AuthorityError::ParentInactive)
    );
    drop(authority);
    let mut authority =
        Authority::open(&ledger, "policy:v2", "generation:01", "state:9", 200).unwrap();
    ...
    assert_eq!(
        authority
            .evaluate_peer(&reopened_channel, &enforced)
            .unwrap()
            .denial_code
            .as_deref(),
        Some("UNAUTHORIZED")
    ...
    assert_eq!(
        ...
        (
            &"activation:parent".into(),
            &"objective:01".into(),
            &"resource/group/item/1".into(),
            &"write".into()
        )
    );
...
fn a_revoked_matching_grant_cannot_mask_an_independent_current_grant() {
    ...
    let request = Invocation::new(
        ...
        ActivationId::new("activation:subject").unwrap(),
        "ReadCap",
        "read",
        "opaque/item",
        10,
...
fn revoking_a_parent_invalidates_an_already_issued_child_chain() {
    ...
    let request = Invocation::new(
        ...
        MachineId::new("machine:01").unwrap(),
        ServiceId::new("service:runtime").unwrap(),
        ActivationId::new("activation:child").unwrap(),
        "ToolCap",
        "read",
        "resource/group/item/1",
        200,
    ...
    authority
        ...
        .unwrap();
    authority
        .issue(parent(), IndependentApproval::verified("operator:1"))
        .unwrap();
    let child = Grant::builder(
        "grant:child",
        "activation:parent",
...
fn reopening_uses_live_configuration_not_serialized_currentness() {
    ...
    let ledger = temp.path().join("authority.json");
    let mut authority =
        Authority::open(&ledger, "policy:old", "generation:01", "state:old", 10).unwrap();
    let request = Invocation::new(
        "command:1",
        MachineId::new("machine:01").unwrap(),
        ServiceId::new("service:runtime").unwrap(),
    ...
    drop(authority);
    let mut reopened =
        Authority::open(&ledger, "policy:new", "generation:02", "state:new", 200).unwrap();
    assert_eq!(
        reopened.advance_time(199),
        Err(AuthorityError::TimeRollback)
    );
    ...
    assert_eq!(
        reopened.bind_peer(
            &forged_channel,
            &request.machine,
            &ServiceId::new("service:attacker").unwrap(),
            &request.activation
        ),
        Err(AuthorityError::PeerCredential)
    );
    let (reopened_channel, _reopened_peer) = UnixStream::pair().unwrap();
    reopened
        .bind_peer(
    ...
    assert_eq!(
        reopened
            .evaluate_peer(&reopened_channel, &request)
            .unwrap()
            .denial_code
            .as_deref(),
        Some("STALE")
    ...
    drop(reopened);
    let mut durable =
        Authority::open(&ledger, "policy:newer", "generation:03", "state:newer", 100).unwrap();
