...
fn revocation_outage_self_authority_and_enforcement_bypass_fail_closed() {
    ...
    authority.issue(parent(), IndependentApproval::verified("operator:01")).unwrap();
    assert_eq!(authority.evaluate_local(&request).unwrap().denial_code.as_deref(),
               Some("UNAUTHORIZED"));
    let enforced = request.clone().with_enforcement(EnforcementProof::verified("lsm:habitat"));
    assert!(authority.evaluate_local(&enforced).unwrap().allowed);
    ...
    drop(authority);
    let mut authority = Authority::open(&ledger, "policy:v2", "generation:01", "state:9",200).unwrap();
    assert_eq!(authority.evaluate_local(&enforced).unwrap().denial_code.as_deref(), Some("UNAUTHORIZED"));
    authority.set_available(false);
    assert_eq!(authority.evaluate_local(&enforced).unwrap().denial_code.as_deref(), Some("UNAVAILABLE"));
...
fn a_revoked_matching_grant_cannot_mask_an_independent_current_grant() {
    ...
    let decision=authority.evaluate_local(&request).unwrap();
...
fn revoking_a_parent_invalidates_an_already_issued_child_chain() {
    ...
    assert_eq!(authority.evaluate_local(&request).unwrap().denial_code.as_deref(),Some("UNAUTHORIZED"));
}
...
fn reopening_uses_live_configuration_not_serialized_currentness(){
    ...
    assert_eq!(reopened.evaluate_local(&request).unwrap().denial_code.as_deref(),Some("STALE"));
}
