...
impl Authority {
    ...
    fn peer_credentials(channel: &UnixStream) -> Result<((u64, u64), libc::ucred), AuthorityError> {
        ...
        Ok(((stat.st_dev as u64, stat.st_ino as u64), unsafe {
            credential.assume_init()
        }))
    }
    pub fn bind_peer(
        &mut self,
    ...
    fn chain_denial(
        ...
        loop {
            ...
            match &current.parent_grant_id {
                Some(parent) => match self.grants.get(parent) {
            ...
            .map(|value| {
                value.channel == observed.0
                    ...
                    && self
                        .trusted_peer
                        .as_ref()
                        .map(|trusted| trusted.executable == executable_identity)
                        .unwrap_or(false)
            })
            .unwrap_or(false);
